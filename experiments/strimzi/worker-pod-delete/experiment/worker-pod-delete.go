...
import (
	...
	litmusLIB "github.com/litmuschaos/litmus-go/chaoslib/litmus/strimzi-worker-delete/lib"
...
func WorkerPodDelete(clients clients.ClientSets) {
	...
	// Including the litmus lib for worker deletion
	switch experimentsDetails.Control.ChaosLib {
	case "litmus":
		if err := litmusLIB.PrepareWorkerPodDelete(&experimentsDetails, clients, &resultDetails, &eventsDetails, &chaosDetails); err != nil {
	...
	// POST-CHAOS check of pod
	if chaosDetails.DefaultAppHealthCheck {
		log.Info("[Status]: Verify that the Kafka Connect cluster is healthy(post-chaos)")
		if err := status.CheckApplicationStatus(experimentsDetails.App.Namespace, chaosDetails.AppDetail.Label, experimentsDetails.Control.Timeout, experimentsDetails.Control.Delay, clients); err != nil {
			log.Errorf("Cluster health check failed, err: %v", err)
			failStep := "[post-chaos]: Failed to verify that the Kafka Connect cluster is healthy, err: " + err.Error()
